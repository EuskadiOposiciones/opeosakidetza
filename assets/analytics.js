...
(function () {
  ...
  var CATEGORY_SLUGS = {
    ...
    'enfermeria': 'enfermeria', 'radiodiagnostico': 'radiodiagnostico', 'laboratorio': 'laboratorio',
    'enfermeria-salud-mental': 'enfermeria_salud_mental', 'matrona': 'matrona',
    'auxiliar-farmacia': 'auxiliar_farmacia', 'psicologia-clinica': 'psicologia_clinica',
    'tecnico-especialista-informatica': 'tecnico_especialista_informatica',
    'tecnico-superior-economico': 'tecnico_superior_economico',
    'tecnico-superior-juridico': 'tecnico_superior_juridico',
  ...
  function pageContext() {
    ...
    return { page_type: pageType, category: category || 'none', page_path: path, page_slug: last || 'home' };
  }
  ...
  function capture(name, props) {
    ...
    window.posthog.capture(name, Object.assign(pageContext(), props || {}));
  }
  ...
  playLinks.forEach(function (a) {
    try {
      ...
      var ctx = pageContext();
      var sourceHost = (window.location.hostname || 'unknown').replace(/^www\./, '');
      referrer.set('utm_source', sourceHost);
      ...
      referrer.set('page_slug', ctx.page_slug);
      if (ctx.page_type === 'phase_2' || ctx.category !== 'none') referrer.set('phase', '2');
  ...
  // Track true CTA exposure once per link and page view, so click-through rate can be calculated.
  if ('IntersectionObserver' in window && playLinks.length) {
    var seenPlay = new WeakSet();
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting && entry.intersectionRatio >= 0.5 && !seenPlay.has(entry.target)) {
          seenPlay.add(entry.target);
          capture('play_cta_viewed', { cta_position: positionOf(entry.target), cta_label: labelOf(entry.target), destination: 'google_play' });
          observer.unobserve(entry.target);
        }
      });
    }, { threshold: [0.5] });
    playLinks.forEach(function (a) { observer.observe(a); });
  }

  document.addEventListener('click', function (ev) {
    var a = ev.target && ev.target.closest ? ev.target.closest('a[href]') : null;
    if (!a) return;
    var abs; try { abs = new URL(a.href, window.location.href); } catch (e) { return; }
    if (abs.hostname === 'play.google.com' && abs.pathname.indexOf('/store/apps/details') !== -1) {
      capture('google_play_clicked', { cta_position: positionOf(a), cta_label: labelOf(a), destination: 'google_play' }); return;
    }
    if (abs.hostname === 'www.osakidetza.euskadi.eus' || abs.hostname === 'osakidetza.euskadi.eus' || abs.hostname === 'www.euskadi.eus' || abs.hostname === 'euskadi.eus' || abs.hostname === 'www.irekia.euskadi.eus') {
      var isPdf = /\.pdf(?:$|\?)/i.test(abs.pathname + abs.search);
      capture('official_source_clicked', {link_position:positionOf(a),link_label:labelOf(a),resource_type:isPdf?'pdf':'portal',source_kind:isPdf?'official_battery_or_document':'official_portal',source_path:abs.pathname.slice(0,300)}); return;
    }
    if (abs.hostname === window.location.hostname) {
      var targetPath = cleanPath(abs.pathname); var targetLast = targetPath.split('/').filter(Boolean).pop() || '';
      if (CATEGORY_SLUGS[targetLast]) capture('category_link_clicked', {target_category:CATEGORY_SLUGS[targetLast],link_position:positionOf(a),link_label:labelOf(a)});
    }
  }, { passive: true });

  var reached = {50:false,90:false};
  function onScroll(){var doc=document.documentElement;var max=Math.max(1,doc.scrollHeight-window.innerHeight);var pct=Math.round((window.scrollY/max)*100);[50,90].forEach(function(threshold){if(!reached[threshold]&&pct>=threshold){reached[threshold]=true;capture('scroll_depth_reached',{depth_percent:threshold});}});if(reached[50]&&reached[90])window.removeEventListener('scroll',onScroll);}
  window.addEventListener('scroll', onScroll, { passive: true });

  document.addEventListener('ope:studyplan', function (ev) {
    var d=(ev&&ev.detail)||{}; capture('study_plan_calculated',{study_plan_category:String(d.category||'manual').slice(0,60),questions_total:Number(d.questions_total||0),days_until_exam:Number(d.days_until_exam||0),full_rounds:Number(d.full_rounds||0),questions_per_day:Number(d.questions_per_day||0)});
  });
})();
